...
namespace MMA.WebApi.Controllers
{
    ...
    public class MazayasubcategoryController : BaseController
    {
        ...
        public async Task<IActionResult> Get()
        {
            try
            {
                var list = await _mazayasubcategoryService.GetMazayasubCategories();

                      foreach (var subcategory in list)
                      {
                        string[] resultArray = subcategory.MazayaCategory.facilities.Split(',');
                        subcategory.MazayaCategory.facilitiesarray = resultArray;
                      }
                return Ok(list);
