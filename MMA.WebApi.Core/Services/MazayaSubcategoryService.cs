...
namespace MMA.WebApi.Core.Services
{
    public class MazayaSubcategoryService : IMazayaSubCategoryService
    {
        ...
        {
            var query = _mazayasubcategoryRepository.GetMazayasubCategoriesNumber().OrderBy(subcategory => subcategory.sort_order).ToList(); ;
            return await Task.FromResult(query.ToList());
        }
        ...
        public async Task<Maybe<MazayaSubCategoryModel>> UpdateCountAsync(MazayaSubCategoryModel model, string userId)
        {
            
                var auditVisitor = new CreateAuditVisitor(userId, DateTime.UtcNow);
                var collection = await _mazayasubcategoryRepository.UpdateCountAsync(model, auditVisitor);
                return collection;
